<?php foreach ($lists as $list) : ?>
	<div class="list_zone" id="<?= $list['id'] ?>">
		<div class="list_item" data-list="<?= $list['id'] ?>">
			<div class="adden_list">
				<span spellcheck="false" data-title="<?= $list['id'] ?>" class="title no_focus"><?= $list['title'] ?></span>
				<div class="close_action">
					<div class="actions" data-id=""><i class="fas fa-ellipsis-h"></i></div>
					<div class="action">
						<p class="action_del">Удалить</p>
					</div>
				</div>
			</div>
			<div class="tasks" data-list="<?= $list['id'] ?>">
				<?php foreach ($tasks as $task) :
					if ($task['list_id'] == $list['id']) : ?>
						<div class="task_item open_edit_task js-open-modal" id="<?= $task['id'] ?>" data-modal="<?= $task['id'] ?>" data-title="<?= $task['title'] ?>" data-desc="<?= $task['description'] ?>" data-cover="<?= $task['cover'] ?>" data-typecover="<?= $task['type_cover'] ?>" data-iduser="<?php echo $_COOKIE['id']; ?>" data-avatar="<?php echo $userdata[0]['avatar']; ?>">
							<?php if ($task['cover'] != '1') { ?>
												...
												<div class="back_two">
													...
													<div class="icon_task">
														...
														<div class="left_icons">
															...
															?>
																<span class="span_clip"><i class="fa-thin fa-comment"></i> <?php echo $count_comments; ?></span>
															<?php } ?>
															<?php
															$coun_checklists = 0;
			...
			<div class="form" data-form="<?= $list['id'] ?>">
				<textarea class="textarea" placeholder="Введите название карточки"></textarea>
				<div class="buttons">
					<button class="add_item">Добавить</button>
					<button class="cancel_item">Отмена</button>
				</div>
			</div>
			<div class="add_task"><span> + </span> Добавить карточку</div>
		</div>
	</div>
<?php endforeach; ?>